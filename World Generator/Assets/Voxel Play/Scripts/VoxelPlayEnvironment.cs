...
namespace VoxelPlay {
	...
	public partial class VoxelPlayEnvironment : MonoBehaviour {
		...
		public float seeThroughHeightOffset = 2f;
		...
		public bool enableSeeThroughVolumes;
		...
		public bool RayCast (Ray ray, out VoxelHitInfo hitInfo, float maxDistance = 0, int minOpaque = 0) {
			return RayCastFast (ray.origin, ray.direction, out hitInfo, maxDistance, false, (byte)minOpaque);
		}
		...
		public Vector3 GetChunkPosition (Vector3 position) {
			int x, y, z;
			FastMath.FloorToInt (position.x / 16f, position.y / 16f, position.z / 16f, out x, out y, out z);
			x = x * 16 + 8;
			y = y * 16 + 8;
			z = z * 16 + 8;
		...
		public int GetVoxelIndices (Vector3 boxMin, Vector3 boxMax, List<VoxelIndex> indices, byte minOpaque = 0, int hasContents = 1) {
			...
			for (float y = chunkMinPos.y; y <= chunkMaxPos.y; y += 16) {
				...
				int voxelIndexMin = 0;
				if (y == chunkMinPos.y) {
					int optimalMin = (int)(boxMin.y - (chunkMinPos.y - 8)) * ONE_Y_ROW;
					if (optimalMin > 0) {
						voxelIndexMin = optimalMin;
					}
				}
				int voxelIndexMax = 16 * 16 * 16;
				if (y == chunkMaxPos.y) {
					int optimalMax = (int)(boxMax.y - (chunkMaxPos.y - 8) + 1) * ONE_Y_ROW;
					if (optimalMax < 16 * 16 * 16) {
		...
		/// <param name="chunk">Chunk.</param>
		/// <param name="voxelIndex">Voxel index.</param>
		public void VoxelSetHidden (VoxelChunk chunk, int voxelIndex, bool hidden, HiddenFilter hiddenFilter = HiddenFilter.OnlySeeThroughAllowedVoxels) {
			if (chunk == null)
				return;

			VoxelSetHiddenOne (chunk, voxelIndex, hidden, hiddenFilter);

			ChunkRequestRefresh (chunk, false, true);
		}

		/// <summary>
		/// Sets if all voxels in a chunk are hidden or not. By default, all voxels are visible. Hiding voxels is not permanent.
		/// </summary>
		/// <param name="chunk">Chunk.</param>
		public void VoxelSetHidden (VoxelChunk chunk, bool hidden, HiddenFilter hiddenFilter = HiddenFilter.OnlySeeThroughAllowedVoxels) {
			if (chunk == null)
				return;
			if (hidden) {
				...
				hiddenVoxel.hidden = true;
				for (int k = 0; k < chunk.voxels.Length; k++) {
					if (chunk.voxels [k].hasContent == 1 && (hiddenFilter == HiddenFilter.AnyVoxel || voxelDefinitions [chunk.voxels [k].typeIndex].seeThroughMode != SeeThroughMode.None)) {
						chunk.voxelsExtraData.Add (k, hiddenVoxel, true);
					}
				}
			} else {
				if (chunk.voxelsExtraData == null)
					return;
				chunk.voxelsExtraData.Clear ();
			}
		...
		public void VoxelSetHidden (List<VoxelIndex> indices, bool hidden, HiddenFilter hiddenFilter = HiddenFilter.OnlySeeThroughAllowedVoxels) {
			if (indices == null)
				return;
			...
			for (int k = 0; k < count; k++) {
				VoxelIndex v = indices [k];
				if (v.chunk != null) {
					VoxelSetHiddenOne (v.chunk, v.voxelIndex, hidden, hiddenFilter);
					if (v.chunk != lastChunk) {
						lastChunk = v.chunk;
		...
		/// </summary>
		/// <param name="indices">List of voxel indices.</param>
		public void VoxelSetHidden (VoxelIndex[] indices, int count, bool hidden, HiddenFilter hiddenFilter = HiddenFilter.OnlySeeThroughAllowedVoxels) {
			if (indices == null)
				return;
			...
			for (int k = 0; k < count; k++) {
				...
				if (chunk != null) {
					VoxelSetHiddenOne (chunk, indices [k].voxelIndex, hidden, hiddenFilter);
					if (chunk != lastChunk) {
						lastChunk = chunk;
						ChunkRequestRefresh (chunk, false, true);
					}
		...
		public bool VoxelIsHidden (VoxelChunk chunk, int voxelIndex) {
			if (chunk == null || chunk.voxelsExtraData == null)
				return false;
			return chunk.voxelsExtraData.ContainsKey (voxelIndex);
		}
		...
		public int VoxelDamage (Vector3 origin, int damage, int radius, bool attenuateDamageWithDistance, bool addParticles = true, List<VoxelIndex> damagedVoxels = null) {
			return DamageAreaFast (origin, damage, radius, attenuateDamageWithDistance, addParticles, damagedVoxels);
		}
		...
		public void ModelPlace (Vector3 position, ModelDefinition model, int rotationDegrees = 0, float colorBrightness = 1f, bool fitTerrain = false, List<VoxelIndex> indices = null) {
			Bounds dummyBounds = new Bounds ();
			ModelPlace (position, model, ref dummyBounds, rotationDegrees, colorBrightness, fitTerrain, indices);
		}
		...
		public void ModelPlace (Vector3 position, ModelDefinition model, out Bounds bounds, int rotationDegrees = 0, float colorBrightness = 1f, bool fitTerrain = false, List<VoxelIndex> indices = null) {
			...
			ModelPlace (position, model, ref mBounds, rotationDegrees, colorBrightness, fitTerrain, indices);
			bounds = mBounds;
		}
