...
namespace VoxelPlay {
	...
	public partial class VoxelPlayThirdPersonController : VoxelPlayCharacterControllerBase {
		...
		void UpdateSimple () {
			...
			if (!overUI) {
				fire1Pressed = input.GetButton (InputButtonNames.Button1);
				fire2Clicked = input.GetButtonDown (InputButtonNames.Button2);
			}
			...
			if (input.focused && input.enabled) {
				...
				if (!leftShiftPressed && !leftAltPressed && !leftControlPressed) {
					if (fire1Pressed) {
						...
						if (fire1Pressed && Time.time - lastHitButtonPressed > player.hitDelay) {
							DoHit (player.hitDamage);
						}
					}
