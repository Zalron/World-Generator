...
//			#endif
		...
		int CreateSeeThroughVariantVoxelDefinition (VoxelDefinition original, float alpha) {
			VoxelDefinition clone = Instantiate<VoxelDefinition> (original);
			clone.name = original.name + " Transparent";
			clone.seeThroughMode = SeeThroughMode.None;
