...
namespace VoxelPlay {
	...
	public class VoxelPlayEnvironmentEditor : Editor {
		...
		#endif
		SerializedProperty enableColliders, enableTrees, denseTrees, enableVegetation, enableNavMesh, hideChunksInHierarchy, enableSeeThroughVolumes;
		...
		void OnEnable () {
			...
			seeThroughHeightOffset = serializedObject.FindProperty ("seeThroughHeightOffset");
			...
			enableSeeThroughVolumes = serializedObject.FindProperty ("enableSeeThroughVolumes");
...
//			bool updateOutlineProperties = false;
			...
			if (world.objectReferenceValue != null) {
				...
				prevBool = editorDraftMode.boolValue;
			...
			if (expandQualitySection) {
				...
				if (seeThrough.boolValue) {
					...
					EditorGUILayout.PropertyField (seeThroughAlpha, new GUIContent ("Alpha", "Transparency of see-through voxels when their see-through setting at voxel definition level is set to Transparency."));
					EditorGUI.indentLevel--;
				}
			}
			// Voxel Generation
			...
			if (expandVoxelGenerationSection) {
				...
				EditorGUILayout.PropertyField (adjustCameraFarClip, new GUIContent ("Adjust Camera Far Clip", "Adjusts camera's far clipping plane to visible chunk distance automatically."));
				...
				if (enableVegetation.boolValue != prevBool)
					rebuildWorld = true;
				EditorGUILayout.PropertyField (enableSeeThroughVolumes, new GUIContent ("See-Through Volumes", "Globally enables/disables generation of see-through volumes when placing models like buildings."));
