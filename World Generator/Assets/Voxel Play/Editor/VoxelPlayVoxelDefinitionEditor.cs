...
namespace VoxelPlay {
	...
	public class VoxelPlayVoxelDefinitionEditor : Editor {
		...
		SerializedProperty biomeDirtCounterpart, seeThroughMode, seeThroughVoxel, seeThroughAlphaMultiplier;
		...
		void OnEnable () {
			...
			seeThroughAlphaMultiplier = serializedObject.FindProperty ("seeThroughAlphaMultiplier");
		}
		...
		public override void OnInspectorGUI () {
			...
			if (((RenderType)renderType.intValue).supportsSeeThrough ()) {
				...
				if (seeThroughMode.intValue == (int)SeeThroughMode.ReplaceVoxel) {
					...
					EditorGUI.indentLevel--;
				} else if (seeThroughMode.intValue == (int)SeeThroughMode.Transparency) {
					EditorGUI.indentLevel++;
					EditorGUILayout.PropertyField (seeThroughAlphaMultiplier, new GUIContent ("Alpha Multiplier", "The transparency of see-through effect is determined by the global see-through alpha setting in Voxel Play Environment multiplied by this factor."));
					EditorGUI.indentLevel--;
				}
			}
